...
Metric counters are bumped here, best-effort: a cache hit (positive *or* negative — both
served without touching Postgres) and a cache miss. Observability must never break the
redirect, so a counter hiccup is swallowed and logged. The served-redirect counter
belongs to the router, since only an actual 302 should bump it.
"""
