...
PUBLIC_HOST = "link-shrink.org"  # tracks the shared ``test_settings`` public host (guarded below)
...
def test_public_host_constant_tracks_shared_settings(test_settings) -> None:
    # Drift guard: the module's PUBLIC_HOST (used in URL assertions and the pure
    # build_qr_payload unit test, which takes no fixture) must stay equal to the shared
    # test_settings public host, so changing one without the other fails loudly here.
    assert PUBLIC_HOST == test_settings.public_host
